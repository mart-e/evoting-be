...
*
*/
int ui_init(int hirq, void (_interrupt _far *irq_handler)(), UI *ui)
{
	...
	_disable();
	ui->old_irq_vector = _dos_getvect(ui->sirq);    // Save old vector
	_dos_setvect(ui->sirq, irq_handler);// And set to caller's function
	_enable();
...
void ui_exit(UI *ui)
{
	...
	_disable();
	_dos_setvect(ui->hirq, ui->old_irq_vector); // Restore old function
	_enable();
